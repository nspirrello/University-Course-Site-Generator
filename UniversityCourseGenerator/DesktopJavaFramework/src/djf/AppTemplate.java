...
public abstract class AppTemplate extends Application {
	...
	try {
	    ...
	    // AND THE CUSTOM UI STUFF FOR THE WORKSPACE
            Alert languageSetting = new Alert(AlertType.CONFIRMATION);
            languageSetting.setContentText("Select a language:");
            languageSetting.setHeaderText("Language Selection");
            languageSetting.setTitle("Language Setting for Course Site Generator");
            ButtonType english = new ButtonType("English");
            ButtonType italian = new ButtonType("Italian");
            languageSetting.getButtonTypes().setAll(english,italian);
            Optional<ButtonType> result = languageSetting.showAndWait();
            
            
            if(result.get() == english){
                AppStartupConstants.APP_PROPERTIES_FILE_NAME = "app_properties.xml";
            }
            if(result.get() == italian){
                AppStartupConstants.APP_PROPERTIES_FILE_NAME = "app_properties_italian.xml";
            }
